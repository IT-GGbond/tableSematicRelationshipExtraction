...
2. **安装依赖**
   ...
   ```
   *(注：在飞桨中，建议先执行 `pip uninstall -y paddlenlp aistudio-sdk` 删掉旧版，再进行安装。)*
...
```
*(注意：`train_dir`需要指向您解压好后的训练集csv文件夹。如果您还没准备好数据，请将数据集下载后放到 `dataset/Train_Set` 里面)*
...
- **做法：** 在将数据输入 Tokenizer 前，把两列拼成自然的句式，例如：
  *原来：* `Jobcenter [SEP] Enzo Cormann`
  *改进后：* `[Subject] Jobcenter and [Object] Enzo Cormann have a relation of [MASK].` 
